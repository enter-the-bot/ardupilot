...
AP_InertialSensor_LSM9DS1::AP_InertialSensor_LSM9DS1(AP_InertialSensor &imu) :
    ...
    _accel_filter(952, 15),  //cutoff 100??
    _gyro_filter(952, 15),
 ...
 *    detect the sensor
*/
AP_InertialSensor_Backend *AP_InertialSensor_LSM9DS1::detect(AP_InertialSensor &_imu)
{
    AP_InertialSensor_LSM9DS1 *sensor = new AP_InertialSensor_LSM9DS1(_imu);
...
{
    _spi = hal.spi->device(AP_HAL::SPIDevice_LSM9DS1);
    ...
    do {
        if (_hardware_init()) {
            ...
            if (!_spi_sem->take(100)) {
                hal.console->printf("LSM9DS1: Unable to get semaphore\n");
                return false;
            }
            ...
            if (a_ready && g_ready) {
                ...
                hal.console->printf("LSM9DS1 startup failed: no data ready\n");
            }
            _spi_sem->give();
        }
        if (tries++ > 5) {
    ...
    hal.scheduler->register_timer_process(FUNCTOR_BIND_MEMBER(&AP_InertialSensor_LSM9DS1::_poll_data, void));

    _gyro_instance = _imu.register_gyro();
    _accel_instance = _imu.register_accel();

    _product_id = AP_PRODUCT_ID_LSM9DS1;
...
*/
void AP_InertialSensor_LSM9DS1::_register_write_xg(uint8_t reg, uint8_t val)
{
    uint8_t tx[2];
    uint8_t rx[2];
...
/*
 *    read from the accelerometer data registers
*/
void AP_InertialSensor_LSM9DS1::_accel_raw_data(struct sensor_raw_data *raw_data)
{
    struct __attribute__((packed)) {
        uint8_t reg;
        struct sensor_raw_data data;
    }rx, tx = { reg : LSM9DS1XG_OUT_X_L_XL | 0x80, };
    _spi->transaction((uint8_t *)&tx, (uint8_t *)&rx, sizeof(rx));
    *raw_data = rx.data;
}

/*
 *    read from the gyroscope data registers
*/
void AP_InertialSensor_LSM9DS1::_gyro_raw_data(struct sensor_raw_data *raw_data)
{
    struct __attribute__((packed)) {
        uint8_t reg;
        struct sensor_raw_data data;
    }rx, tx = { reg : LSM9DS1XG_OUT_X_L_G | 0x80, };

    _spi->transaction((uint8_t *)&tx, (uint8_t *)&rx, sizeof(rx));
    *raw_data = rx.data;
}

void AP_InertialSensor_LSM9DS1::_read_data_transaction_a()
{
...
/*
 *  set the accelerometer filter frequency
 */
void AP_InertialSensor_LSM9DS1::_set_accel_filter(uint8_t filter_hz)
{
    _accel_filter.set_cutoff_frequency(952, filter_hz);
}

/*
 *  set the gyroscope filter frequency
 */
void AP_InertialSensor_LSM9DS1::_set_gyro_filter(uint8_t filter_hz)
{
    _gyro_filter.set_cutoff_frequency(952, filter_hz);
}
